...
# ================================================================
# 13. GLOBAL ELASTIC STIFFNESS
#
# Because all elements have:
#
#   same geometry
#   same elastic properties
#
# the same element stiffness can be reused.
# ================================================================
...
def solve_cp_local(
    ...
    # Limit unreasonable starting values
    ...
    for iteration in range(
        ...
        # Hardening
        #
        # Per-slip hardening as in v02 5B.
        # --------------------------------------------------------
        ...
        # Explicitly force zero at tau = 0
        ...
        if (
            ...
            # Recalculate RSS from final stress
        ...
        for a in range(12):
            ...
            # dv/dtau = n*v/tau
            #
            # For tau -> 0, derivative is zero for n > 1.
            # ----------------------------------------------------

            if abs(tau_a) > 1.0e-14:
            ...
            # If the current resistance has reached saturation,
            ...
            if (
                ...
                if x[a] > 0.0:
                    ...
                    # At x=0 use zero subgradient.
                    sign_x = 0.0

                dg_dx = (
                    hardening_prefactor[a]
                    *
                    sign_x
                )
            ...
            # ----------------------------------------------------
            # Derivative of slip rate
            #
            # d tau_a / d x_b = -A_ab
            #
            # resistance only depends on x_a.
            # ----------------------------------------------------
        ...
        while (
            ...
            resistance_trial = np.minimum(
                resistance_trial,
                TAU_SAT
            )

            resistance_trial = np.maximum(
                resistance_trial,
                TAU0
            )
            ...
            ratio_trial = (
                np.abs(tau_trial_new)
                /
                resistance_trial
            )

            ratio_trial = np.minimum(
                ratio_trial,
                1.0e3
            )
...
while (
    ...
    for global_iter in range(
        ...
        # IMPORTANT:
        #
        # The current solver uses the elastic K as the global
        # tangent, exactly as the existing v02 5B formulation.
        #
        # We factorize it only once.
        # ========================================================
...
np.savez_compressed(
    "316L_FE_CPFEM_optimized_results_v02.npz",

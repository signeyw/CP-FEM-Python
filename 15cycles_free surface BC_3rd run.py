...
# ============================================================
# 5. Generate NEW random crystallographic orientations
#    while keeping the Voronoi geometry IDENTICAL
# ============================================================

# ORIENTATION_SEED = 20260912
...
microstructure_file = (
    f"316L_3D_Voronoi_microstructure_{REALIZATION}.npz"
)
...
micro_data = np.load(microstructure_file)

grain_orientation = micro_data["orientation_matrices"]

grain_euler = micro_data["euler_angles"]
...
tau_sat = 450.0       # Saturation resistance, MPa
...
# ============================================================
# ------------------------------------------------------------
# Algorithmic tangent sparse solver
# ------------------------------------------------------------
...
MAX_CYCLIC_STRAIN = 0.0020       # +0.20%
R_RATIO = 0.1
MIN_CYCLIC_STRAIN = (
    R_RATIO * MAX_CYCLIC_STRAIN
)                                  # +0.02%

# One complete tensile-tensile cycle:
#
# 0.00% -> +0.20% -> +0.02% -> +0.20%
#
# N_CYCLES = 15

INITIAL_STRAIN_STEP = 5.0e-4     # 0.05%
MIN_STRAIN_STEP = 5.0e-5         # 0.005%
MAX_STRAIN_STEP = 1.0e-3         # 0.10%


# ================================================================
# FATIGUE INDICATOR / FIP SETTINGS
# ================================================================

# Fatemi-Socie normal-stress sensitivity.
#
# Set to zero for the first run so that the raw shear-slip
# contribution can be examined independently.
#
# This is NOT a calibrated material constant yet.
FS_K = 0.0

# Reference stress used by the FS normal-stress correction.
# Keep separate from TAU0 because this is a fatigue-model
# reference parameter, not the crystal slip resistance.
FS_SIGMA_REF = TAU0

# Brown-Miller weighting coefficient.
#
# Set to 1.0 initially so that the raw combined indicator
# can be examined. It should be calibrated later.
BM_C = 1.0

# SWT uses stress × strain and therefore has units of MPa.
# No fitting parameter is required.
...
#       S = 1/2 (s⊗n + n⊗s)
#
# where n and s are unit vectors.
# ================================================================
...
#     # Three <110> directions in each plane.
#     #
#     # Each direction is explicitly checked for
#     # orthogonality to its plane.
#     directions = [
#         [
...
#                 raise RuntimeError(
#                     "Slip direction and plane "
#                     "normal are not orthogonal."
#                 )
...
# crystal_S = build_fcc_slip_systems()


# if crystal_S.shape != (12, 3, 3):

#     raise RuntimeError(
#         "FCC slip-system construction failed."
#     )
...
def build_fcc_slip_systems():
    ...
    for p, dirs in zip(planes, directions):
    ...
    return (
        np.asarray(systems, dtype=np.float64),
...
if crystal_normals.shape != (12, 3):
    raise RuntimeError(
        "FCC slip-plane normal construction failed."
    )
...
# ================================================================
# 10B. GRAIN / ELEMENT SLIP-PLANE NORMALS
# ================================================================

grain_normals = np.empty(
    (n_grains, 12, 3),
    dtype=np.float64
)
...
element_normals = grain_normals[
    element_grain
]


print(
    "element_normals shape =",
    element_normals.shape
)
...
print(
    "Preparing FE–CPFEM algorithmic tangent..."
)
t0 = time.time()
...
print()
print("Initial elastic tangent factorization...")

t0 = time.time()

# Extract free-free stiffness matrix using sparse indexing
K_ff = (
    K_global
...
def solve_cp_local(
    ...
    # --------------------------------------------------------
    # 1. Convert total and plastic strain to Voigt notation
    # --------------------------------------------------------

    total_eps_voigt = tensor_to_voigt(total_eps)
    plastic_old_voigt = tensor_to_voigt(plastic_old)
    ...
    # Elastic trial stress
    stress_trial_voigt = (
        C_ELASTIC @ elastic_trial_voigt
    )

    # Trial resolved shear stress
    tau_trial = T @ stress_trial_voigt

    # --------------------------------------------------------
    # 2. Initial guess for slip increments
    # --------------------------------------------------------
    ...
    # Protect against invalid initial guesses
    x = np.nan_to_num(
        x,
        nan=0.0,
        posinf=CP_DGAMMA_MAX,
        neginf=-CP_DGAMMA_MAX
    )
    ...
    # --------------------------------------------------------
    # 3. Hardening prefactor
    #
    #     d(g_i)/d(|gamma_i|)
    #
    #     tau_c evolves toward TAU_SAT
    # --------------------------------------------------------
    ...
    # --------------------------------------------------------
    # 4. Local Newton iteration
    # --------------------------------------------------------

    converged = False
    rnorm = np.inf

    for iteration in range(CP_MAX_ITER):
        ...
        # ----------------------------------------------------
        # Current CRSS / resistance
        # ----------------------------------------------------

        resistance_new = (
            resistance_old
            + hardening_prefactor * np.abs(x)
        )

        resistance_new = np.clip(
            resistance_new,
            TAU0,
            TAU_SAT
        )
        ...
        # * (|tau|/CRSS)^n
        #
        # The ratio cap prevents overflow.
        # ----------------------------------------------------
        ...
        mask_resistance = (
            resistance_new > 1.0e-14
        )

        ratio[mask_resistance] = (
            np.abs(tau[mask_resistance])
            / resistance_new[mask_resistance]
        )
        ...
        # R = x/dt - gamma_dot = 0
        #
        # Normalized by GAMMA0 for numerical conditioning.
        # ----------------------------------------------------
        ...
        if (
            ...
            #   d(gamma_dot)/d(gamma)
            #
            # followed by the local Jacobian and
            # d(gamma)/d(epsilon).
            #
            # =================================================
            ...
            # ------------------------------------------------
            # Derivative of slip rate with respect to RSS
            #
            # d(gamma_dot)/d(tau)
            #
            # = n * gamma_dot / tau
            ...
            # ------------------------------------------------
            # Derivative of slip rate with respect to CRSS
            #
            # d(gamma_dot)/d(g)
            #
            # = -n * gamma_dot / CRSS
            ...
            dv_dg[mask_resistance] = (
                -RATE_EXPONENT
                * rates[mask_resistance]
                / resistance_new[mask_resistance]
            )
            ...
            dg_dx[not_saturated] = (
                hardening_prefactor[not_saturated]
                * np.sign(x[not_saturated])
            )

            # ------------------------------------------------
            # Local derivative of slip rate with respect to
            # slip increment
            #
            # tau = tau_trial - A*x
            #
            ...
            # ------------------------------------------------
            # d(rate)/d(total strain)
            #
            # tau depends on total strain through M:
            #
            # d(tau)/d(epsilon) = M
            # ------------------------------------------------
            ...
            # Numerical symmetry
            #
            # The constitutive tangent is symmetrized to
            # improve robustness of the global FE solver.
            # ------------------------------------------------
        ...
        # ====================================================
        # 5. ANALYTICAL LOCAL JACOBIAN — VECTORIZED
        # ====================================================
        #
        # IMPORTANT:
        #
        # This block MUST NOT be inside:
        #
        #     for a in range(12):
        #
        # because all 12 slip systems are handled
        # simultaneously here.
        #
        # The previous version unnecessarily repeated this
        # entire calculation 12 times per Newton iteration.
        #
        # ====================================================
        ...
        # ----------------------------------------------------
        # Jacobian of normalized residual
        #
        # R = (x/dt - rate)/GAMMA0
        # ----------------------------------------------------
        ...
        # ----------------------------------------------------
        # Limit excessively large Newton corrections
        # ----------------------------------------------------
        ...
        while (
            ...
            ratio_trial[mask_trial] = (
                np.abs(
                    tau_trial_new[mask_trial]
                )
                / resistance_trial[mask_trial]
            )
            ...
            rates_trial = (
                GAMMA0
                * np.sign(tau_trial_new)
                * ratio_trial ** RATE_EXPONENT
            )
            ...
            if (
                np.isfinite(rnorm_trial)
                and rnorm_trial < rnorm
            ):
        ...
        if (
            alpha < CP_LINESEARCH_MIN
        ):

            # Fall back to a very small Newton step
            # rather than allowing invalid values.

            x = np.clip(
                x_old + CP_LINESEARCH_MIN * dx,
                -CP_DGAMMA_MAX,
                CP_DGAMMA_MAX
            )

    # ========================================================
    # 7. Local Newton solver failed to converge
    # ========================================================
...
cyclic_segment_history = []
cyclic_direction_history = []
...
# ================================================================
# STEPWISE CHECKPOINT SAVING
# ================================================================
#
# A separate NPZ file is written after EVERY CONVERGED load step.
#
# This prevents loss of local FE–CPFEM history if Jupyter crashes,
# the kernel is restarted, or the simulation is interrupted.
#
# Only physically converged states are saved.
# ================================================================

import os

# CHECKPOINT_DIR = (
#     "316L_CPFEM_10cycles_stepwise_checkpoints"
# )

CHECKPOINT_DIR = (
    f"316L_CPFEM_15cycles_{REALIZATION}_stepwise_checkpoints"
)
...
print()
print("=" * 70)
print("STEPWISE CHECKPOINT SAVING ENABLED")
print("=" * 70)
print(
...
def save_converged_checkpoint(
    ...
    """
    Save one physically converged FE–CPFEM state.

    IMPORTANT:
    This function is called ONLY after global convergence.

    Therefore no Newton trial states are saved.
    """

    # ------------------------------------------------------------
    # Total strain: engineering Voigt
    # [xx, yy, zz, xy, yz, xz]
    # ------------------------------------------------------------

    total_strain_voigt = np.asarray(
        strain_all,
        dtype=np.float32
    ).copy()

    # ------------------------------------------------------------
    # Convert engineering Voigt strain to tensor
    # ------------------------------------------------------------

    total_strain_tensor = np.zeros(
        (
            len(total_strain_voigt),
            3,
            3
        ),
        dtype=np.float32
    ...
    total_strain_tensor[:, 0, 0] = (
        total_strain_voigt[:, 0]
    )

    total_strain_tensor[:, 1, 1] = (
        total_strain_voigt[:, 1]
    )

    total_strain_tensor[:, 2, 2] = (
        total_strain_voigt[:, 2]
    )

    total_strain_tensor[:, 0, 1] = (
        total_strain_voigt[:, 3] / 2.0
    )
    total_strain_tensor[:, 1, 0] = (
        total_strain_voigt[:, 3] / 2.0
    )

    total_strain_tensor[:, 1, 2] = (
        total_strain_voigt[:, 4] / 2.0
    )
    total_strain_tensor[:, 2, 1] = (
        total_strain_voigt[:, 4] / 2.0
    )

    total_strain_tensor[:, 0, 2] = (
        total_strain_voigt[:, 5] / 2.0
    )
    total_strain_tensor[:, 2, 0] = (
        total_strain_voigt[:, 5] / 2.0
    )
    ...
    np.savez_compressed(
        ...
        cyclic_segment=np.int32(
            cyclic_segment
        ),
        cyclic_direction=np.int32(
            cyclic_direction
        ),

        current_strain=np.float64(
            current_strain
        ),

        segment_target=np.float64(
            segment_target
        ),

        macro_stress=np.float64(
            sigma_macro
        ),

        # --------------------------------------------------------
        # Element / grain identification
        # --------------------------------------------------------

        element_id=np.arange(
            len(element_grain),
            dtype=np.int32
        ),
        ...
        max_abs_RSS=max_abs_rss,

        max_RSS_system=max_rss_system,

        # --------------------------------------------------------
        # Local solver diagnostics
        # --------------------------------------------------------

        CP_residual=np.asarray(
            cp_residual,
            dtype=np.float32
        ).copy(),

        CP_iterations=np.asarray(
            cp_iterations,
            dtype=np.int16
        ).copy(),

        CP_converged=np.asarray(
            cp_converged,
            dtype=bool
        ).copy()
    )
...
while cyclic_segment < len(cyclic_targets):

    # ------------------------------------------------------------
    # Current segment target
    # ------------------------------------------------------------

    segment_target = cyclic_targets[cyclic_segment]
    ...
    if abs(distance_to_target) <= 1.0e-14:

        current_strain = segment_target
    ...
    actual_step = min(
        abs(strain_step),
        abs(distance_to_target)
    )

    # ------------------------------------------------------------
    # Signed target strain
    # ------------------------------------------------------------

    target_strain = (
        current_strain
        + cyclic_direction * actual_step
    )

    # ------------------------------------------------------------
    # Actual signed strain increment
    # ------------------------------------------------------------
    ...
    # CP time increment
    #
    # dt must always remain positive.
    # The loading direction is contained in actual_increment.
    # ------------------------------------------------------------

    cp_dt = (
        abs(actual_increment)
        / STRAIN_RATE
    )
    ...
    # --------------------------------------------------------
    # Reapply fixed bottom boundary conditions
    # --------------------------------------------------------
    ...
    for global_iter in range(
        ...
        # Element strains
        #
        # B is constant because the current formulation uses
        # small-strain HEX8 with one constitutive point.
        #
        # Shape:
        #     (n_elements, 6)
        # ----------------------------------------------------

        strain_all = np.einsum(
            "ij,ej->ei",
            B,
            Ue_all,
            optimize=True
        ...
        for e in range(n_elements):
            ...
            # ------------------------------------------------
            # Local CPFEM constitutive update
            #
            # IMPORTANT:
            # Always use the committed state from the beginning
            # of the current load increment.
            #
            # Do NOT use the previous global Newton iteration's
            # trial state as the new committed state.
            # ------------------------------------------------

            cp_result = solve_cp_local(
                total_eps,
                plastic_backup[e],
                resistance_backup[e],
                element_S[e],
            ...
            # Store local CP convergence status
            ...
            if not cp_ok:
                ...
                print(
                    f"CPFEM LOCAL SOLVER FAILED "
                    f"AT ELEMENT {e}"
                )
                print(
                    f"Global load step : {step}"
                )
                print(
                    f"Global iteration : {global_iter}"
                )
                print(
                    f"CP residual      : {cp_res:.6e}"
                )
                print(
                    f"CP iterations    : {cp_niter}"
                )
                print(
                    "------------------------------------------------"
                )
            ...
            # ------------------------------------------------
            # Accumulated scalar slip
            #
            # This is cumulative over load increments:
            #
            # accumulated slip =
            # previous accumulated slip
            # + sum(abs(dgamma))
            # ------------------------------------------------
            ...
            # ------------------------------------------------------------
            # Per-slip-system accumulated slip
            # ------------------------------------------------------------
            
            trial_accumulated_slip_system[e] = (
                accumulated_slip_system_committed[e]
                + np.abs(dgamma)
            )
        ...
        # ----------------------------------------------------
        # If a local CPFEM calculation failed, stop this
        # global Newton iteration.
        # ----------------------------------------------------
        ...
        # ====================================================
        # ALGORITHMIC TANGENT ASSEMBLY
        # ====================================================

        tangent_start = time.time()

        # ----------------------------------------------------
        # Element algorithmic stiffness
        #
        # Ke = B^T C_alg B * V
        #
        # Shape:
        ...
        # Ke = B^T C_alg B * V
        #
        # MEMORY-SAFE BLOCKED ASSEMBLY
        # ----------------------------------------------------
        
        TANGENT_BLOCK_SIZE = 500
        ...
        for e0 in range(0, n_elements, TANGENT_BLOCK_SIZE):
        
            e1 = min(
                e0 + TANGENT_BLOCK_SIZE,
                n_elements
            )
        
            Ke_alg_all[e0:e1] = np.einsum(
                "ia,eij,jb->eab",
                B,
                trial_C_alg[e0:e1],
                B,
        ...
        # ----------------------------------------------------
        # Sparse global assembly using COO
        # ----------------------------------------------------

        K_global_alg = coo_matrix(
            (
                Ke_alg_all.reshape(-1),
                (
                    row_idx,
                    col_idx
                )
        ...
        #     print()
        #     print("Algorithmic tangent factorization failed:")
        #     print(err)
        #     raise
        
        # print(
        #     f"K_alg factorization = "
        #     f"{time.time() - factor_start:.3f} s"
        # )
        #             global_converged = False
        #             break
    ...
    # ========================================================
    #
    # This section is reached ONLY if global Newton convergence
    # was achieved.
    #
    # Therefore all fields below represent the physical
    # converged state of this load step.
    # ========================================================
    ...
    # ========================================================
    # STORE CONVERGED LOCAL TOTAL STRAIN
    # ========================================================
    ...
    # ========================================================
    # STORE CONVERGED STEP / CYCLE INFORMATION
    # ========================================================
    
    local_field_cycle_history.append(
        int(
            max(
                1,
                int(
                    np.ceil(
                        (cyclic_segment + 1) / 3.0
                    )
                )
            )
        )
    ...
    # --------------------------------------------------------
    # Mean plastic strain magnitude
    #
    # NOTE:
    # This is the Frobenius norm of the plastic strain tensor.
    # It is NOT the J2 equivalent plastic strain.
    # J2 equivalent plastic strain is calculated later.
    # --------------------------------------------------------
    
    plastic_strain_norm = np.sqrt(
        np.einsum(
            "eij,eij->e",
            plastic_strain_committed,
            plastic_strain_committed
    ...
    cyclic_segment_history.append(
        cyclic_segment + 1
    )
    
    cyclic_direction_history.append(
        cyclic_direction
    )
    
    
    # ========================================================
    # STORE GLOBAL CP STATISTICS
    # ========================================================
    ...
    # ========================================================
    # SAVE STEPWISE CONVERGED CHECKPOINT
    # ========================================================
    #
    # IMPORTANT:
    #
    # This is AFTER:
    #   1. Global Newton convergence
    #   2. State commitment
    #   3. Local-field storage
    #   4. Macroscopic stress calculation
    #   5. Global history storage
    #
    # Therefore this checkpoint contains a complete,
    # physically converged state for this load step.
    #
    # Failed Newton iterations are NOT saved.
    # ========================================================
    
    checkpoint_file = save_converged_checkpoint(
    
        step=step,
    
        cycle=local_field_cycle_history[-1],
        ...
        strain_all=trial_total_strain,
    
        stress=trial_stress,
    
        stress_voigt=trial_stress_voigt,
        ...
        cp_residual=trial_cp_residual,
    
        cp_iterations=trial_cp_iterations,
    
        cp_converged=trial_cp_converged,
...
# ================================================================
# FATIGUE INDICATOR / FIP CALCULATION
# ================================================================
#
# All calculations are performed AFTER the converged FE–CPFEM
# simulation using the complete physical load-step history.
#
# Dimensions:
#
#   n_history = number of converged load steps
#   n_elements = 8000
#   n_slip = 12
#
# The FIPs are calculated for every element and every FCC
# slip system.
# ================================================================

print()
print("=" * 70)
print("CALCULATING MULTIPLE FATIGUE INDICATORS")
print("=" * 70)


n_history = local_stress_history.shape[0]
n_slip = 12


# ------------------------------------------------
# 1. Build stress normal to each slip plane
# ------------------------------------------------
#
# sigma_n = n · sigma · n
#
# Shape:
#   (history, element, slip_system)
# ------------------------------------------------

normal_stress_history = np.einsum(
    "heij,eaj,eai->hea",
...
# ------------------------------------------------
# 2. Build normal strain on each slip plane
# ------------------------------------------------
#
# Engineering-strain Voigt:
#
# [xx, yy, zz, gamma_xy, gamma_yz, gamma_xz]
#
# Convert to tensor form first.
# ------------------------------------------------

eps = local_total_strain_history

strain_tensor_history = np.zeros(
    (
        n_history,
        n_elements,
        3,
...
strain_tensor_history[..., 0, 0] = eps[..., 0]
strain_tensor_history[..., 1, 1] = eps[..., 1]
strain_tensor_history[..., 2, 2] = eps[..., 2]

strain_tensor_history[..., 0, 1] = eps[..., 3] / 2.0
strain_tensor_history[..., 1, 0] = eps[..., 3] / 2.0

strain_tensor_history[..., 1, 2] = eps[..., 4] / 2.0
strain_tensor_history[..., 2, 1] = eps[..., 4] / 2.0

strain_tensor_history[..., 0, 2] = eps[..., 5] / 2.0
strain_tensor_history[..., 2, 0] = eps[..., 5] / 2.0


normal_strain_history = np.einsum(
    "heij,eaj,eai->hea",
    strain_tensor_history,
    element_normals,
    element_normals,
    optimize=True
)
...
# ------------------------------------------------
# 3. Signed cumulative slip
# ------------------------------------------------
#
# local_accumulated_slip_system_history contains
# accumulated ABSOLUTE slip.
#
# For fatigue we need signed cyclic slip.
#
# Therefore reconstruct:
#
# gamma_signed(N)
#     = cumulative sum of dgamma
#
# ------------------------------------------------

signed_slip_history = np.cumsum(
    local_slip_increment_history,
    axis=0
)
...
segment_history = np.asarray(
    cyclic_segment_history,
    dtype=np.int32
)

direction_history = np.asarray(
    cyclic_direction_history,
    dtype=np.int32
)

# Segment numbering in cp_state_history starts at 1.
#
# A complete cycle is represented by:
#
#   segment 1: 0       -> maximum
#   segment 2: maximum -> minimum
#   segment 3: minimum -> maximum
#
# Then the next cycle repeats segments 4-6, etc.


cycle_numbers = np.zeros(
    n_history,
    dtype=np.int32
)
...
n_completed_cycles = int(
    np.max(cycle_numbers)
)


print(
    "Number of converged load steps =",
    n_history
)
...
# Raw components are also saved so that the empirical
# coefficients can be changed later without rerunning CPFEM.

fip_FS_shear_history = []
fip_FS_normal_history = []

fip_BM_shear_history = []
fip_BM_normal_history = []


critical_CPS_element_history = []
critical_CPS_grain_history = []
critical_CPS_slip_system_history = []

critical_PD_element_history = []
critical_PD_grain_history = []
critical_PD_slip_system_history = []

critical_FS_element_history = []
critical_FS_grain_history = []
critical_FS_slip_system_history = []

critical_BM_element_history = []
critical_BM_grain_history = []
critical_BM_slip_system_history = []

critical_SWT_element_history = []
critical_SWT_grain_history = []
critical_SWT_slip_system_history = []

critical_J2_element_history = []
critical_J2_grain_history = []
...
for cycle_id in range(
    ...
    sigma_n_cycle = (
        normal_stress_history[
            cycle_indices
        ]
    )

    eps_n_cycle = (
        normal_strain_history[
            cycle_indices
        ]
    )
    ...
    # ============================================================
    # 1. CUMULATIVE PLASTIC SLIP
    # ============================================================
    #
    # Total absolute slip accumulated during this cycle.
    #
    # This is a direct slip-activity indicator.
    # ============================================================

    gamma_increment_cycle = (
        local_slip_increment_history[
            cycle_indices
        ]
    )
    ...
    CPS_max_index = np.unravel_index(
        np.argmax(CPS),
        CPS.shape
    )

    e_CPS, a_CPS = CPS_max_index

    fip_CPS_history.append(
        CPS.copy()
    )

    critical_CPS_element_history.append(
        e_CPS
    )

    critical_CPS_grain_history.append(
        element_grain[e_CPS]
    )

    critical_CPS_slip_system_history.append(
        a_CPS
    )


    # ============================================================
    # 2. FATIGUE PLASTIC DISSIPATION
    # ============================================================
    #
    # Wp = integral sigma : d(eps_p)
    #
    # Trapezoidal integration over converged load steps.
    #
    # Units: MPa
    # ============================================================
    ...
    plastic_work_increment = np.einsum(
        "heij,heij->he",
        sigma_mid,
        dplastic
    )
    ...
    # Use absolute cycle dissipation because numerical
    # integration over a closed loop can contain small
    # sign errors.
    PD = np.abs(PD)

    e_PD = np.argmax(PD)

    fip_PD_cycle = PD.copy()

    fip_PD_history.append(
        fip_PD_cycle
    )

    critical_PD_element_history.append(
        e_PD
    )

    critical_PD_grain_history.append(
        element_grain[e_PD]
    )

    # PD is element-based, not slip-system-based.
    critical_PD_slip_system_history.append(
        -1
    )
    ...
    fip_FS_history.append(
        FS.copy()
    )

    fip_FS_shear_history.append(
        FS_shear.copy()
    )

    fip_FS_normal_history.append(
        FS_normal.copy()
    )

    e_FS, a_FS = np.unravel_index(
        np.argmax(FS),
        FS.shape
    )

    critical_FS_element_history.append(
        e_FS
    )

    critical_FS_grain_history.append(
        element_grain[e_FS]
    )

    critical_FS_slip_system_history.append(
        a_FS
    )
    ...
    #        + C * Delta_epsilon_n
    #
    # We use the absolute normal-strain range.
    #
    # BM_C is explicitly exposed for later calibration.
    # ============================================================
    ...
    fip_BM_history.append(
        BM.copy()
    )

    fip_BM_shear_history.append(
        BM_shear.copy()
    )

    fip_BM_normal_history.append(
        BM_normal.copy()
    )

    e_BM, a_BM = np.unravel_index(
        np.argmax(BM),
        BM.shape
    )

    critical_BM_element_history.append(
        e_BM
    )

    critical_BM_grain_history.append(
        element_grain[e_BM]
    )

    critical_BM_slip_system_history.append(
        a_BM
    )
    ...
    # ============================================================
    #
    # Crystallographic normal-strain version:
    #
    #   SWT = sigma_n,max * Delta_epsilon_n / 2
    #
    # Units: MPa
    # ============================================================
    ...
    fip_SWT_history.append(
        SWT.copy()
    )

    e_SWT, a_SWT = np.unravel_index(
        np.argmax(SWT),
        SWT.shape
    )

    critical_SWT_element_history.append(
        e_SWT
    )

    critical_SWT_grain_history.append(
        element_grain[e_SWT]
    )

    critical_SWT_slip_system_history.append(
        a_SWT
    )
    ...
    # ============================================================
    # 6. CYCLIC J2 PLASTIC STRAIN HALF-RANGE
    # ============================================================
    #
    # Element-level equivalent plastic strain half-range.
    # This provides a continuum-level baseline FIP.
    # ============================================================
    ...
    critical_J2_element_history.append(
        e_J2
    )

    critical_J2_grain_history.append(
        element_grain[e_J2]
    )
    ...
    print(
        f"  Max J2 range/2 = "
        f"{np.max(J2_half_range):.8e}"
    )

    print(
        f"  Critical FS: "
        f"element {e_FS}, "
        f"grain {element_grain[e_FS]}, "
        f"slip system {a_FS}"
    )
...
# ================================================================
# CONVERT FIP HISTORIES TO ARRAYS
# ================================================================

fip_CPS_history = np.asarray(
    fip_CPS_history,
    dtype=np.float64
)

fip_PD_history = np.asarray(
    fip_PD_history,
    dtype=np.float64
)

fip_FS_history = np.asarray(
    fip_FS_history,
    dtype=np.float64
)

fip_BM_history = np.asarray(
    fip_BM_history,
    dtype=np.float64
)

fip_SWT_history = np.asarray(
    fip_SWT_history,
    dtype=np.float64
)

fip_J2_history = np.asarray(
    fip_J2_history,
    dtype=np.float64
)

fip_FS_shear_history = np.asarray(
    fip_FS_shear_history,
    dtype=np.float64
)

fip_FS_normal_history = np.asarray(
    fip_FS_normal_history,
    dtype=np.float64
)

fip_BM_shear_history = np.asarray(
    fip_BM_shear_history,
    dtype=np.float64
)

fip_BM_normal_history = np.asarray(
    fip_BM_normal_history,
    dtype=np.float64
)


critical_CPS_element_history = np.asarray(
    critical_CPS_element_history,
    dtype=np.int32
)

critical_CPS_grain_history = np.asarray(
    critical_CPS_grain_history,
    dtype=np.int32
)

critical_CPS_slip_system_history = np.asarray(
    critical_CPS_slip_system_history,
    dtype=np.int32
)

critical_PD_element_history = np.asarray(
    critical_PD_element_history,
    dtype=np.int32
)

critical_PD_grain_history = np.asarray(
    critical_PD_grain_history,
    dtype=np.int32
)

critical_FS_element_history = np.asarray(
    critical_FS_element_history,
    dtype=np.int32
)

critical_FS_grain_history = np.asarray(
    critical_FS_grain_history,
    dtype=np.int32
)

critical_FS_slip_system_history = np.asarray(
    critical_FS_slip_system_history,
    dtype=np.int32
)

critical_BM_element_history = np.asarray(
    critical_BM_element_history,
    dtype=np.int32
)

critical_BM_grain_history = np.asarray(
    critical_BM_grain_history,
    dtype=np.int32
)

critical_BM_slip_system_history = np.asarray(
    critical_BM_slip_system_history,
    dtype=np.int32
)

critical_SWT_element_history = np.asarray(
    critical_SWT_element_history,
    dtype=np.int32
)

critical_SWT_grain_history = np.asarray(
    critical_SWT_grain_history,
    dtype=np.int32
)

critical_SWT_slip_system_history = np.asarray(
    critical_SWT_slip_system_history,
    dtype=np.int32
)

critical_J2_element_history = np.asarray(
    critical_J2_element_history,
    dtype=np.int32
)

critical_J2_grain_history = np.asarray(
    critical_J2_grain_history,
    dtype=np.int32
)
...
np.savez_compressed(
    ...
    "316L_FE_CPFEM_cyclic_3cycles_stepwise_local_history_v04.npz",
...
# ============================================================
    ...
    fip_plastic_dissipation_history=
        fip_PD_history,

    fip_Fatemi_Socie_history=
        fip_FS_history,

    fip_Fatemi_Socie_shear_history=
        fip_FS_shear_history,

    fip_Fatemi_Socie_normal_factor_history=
        fip_FS_normal_history,

    fip_Brown_Miller_history=
        fip_BM_history,

    fip_Brown_Miller_shear_history=
        fip_BM_shear_history,

    fip_Brown_Miller_normal_history=
        fip_BM_normal_history,

    fip_SWT_history=
        fip_SWT_history,

    fip_J2_plastic_strain_range_history=
        fip_J2_history,

    # ------------------------------------------------------------
    # FIP critical locations
    # ------------------------------------------------------------
    ...
    # ------------------------------------------------------------
    # Crystallographic normal stress / strain
    # ------------------------------------------------------------

    local_normal_stress_history=
        normal_stress_history,

    local_normal_strain_history=
        normal_strain_history,

    local_signed_slip_history=
        signed_slip_history,
